...
function onEvent(name, value1, value2)
    ...
    if name == 'OLDHallucination Boppers' then
		if killbop == 1 then
			...
			setProperty('back1.alpha', 1);
				...
				for index = 0, 7 do 
				noteTweenAlpha(index, index, 0.8, 0.0001, 'linear');
				end
			
				if getProperty('timeTxt') and not downscroll then
				doTweenY('coolthing', 'timeTxt', 10, 0.5, 'smoothStepOut')
			...
			if killbop == 3 then
				...
				for index = 0, 7 do 
				noteTweenAlpha(index, index, 1, 0.0001, 'linear');
				end
				
				if getProperty('timeTxt') and not downscroll then
					doTweenY('finishedcoolthing', 'timeTxt', 50, 0.5, 'smoothStepOut')
