...
class AlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, character:String, bold:Bool, parent:Alphabet)
	{
		super(x, y);
		this.parent = parent;
		//trace(Std.is(Type.getClassName(Type.getClass(options.subState)), BaseOptionsMenu));
	//	if (FlxG.state.subState != null) trace(Type.getSuperClass(Type.getClass(FlxG.state.subState)) == options.OptionsState);
		if (FlxG.state is options.OptionsState) image = 'alphabetBright' else image = 'alphabet';
