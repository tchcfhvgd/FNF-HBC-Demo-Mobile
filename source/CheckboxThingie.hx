	...
	private function set_daValue(check:Bool):Bool {
		if(check) {
			if(animation.curAnim.name != 'checked' && animation.curAnim.name != 'checking') {
				...
				if (!Std.isOfType(FlxG.state, FreeplayState)) offset.set(34, 25);
			}
		} else if(animation.curAnim.name != 'unchecked' && animation.curAnim.name != 'unchecking') {
			animation.play("unchecking", true);
			if (!Std.isOfType(FlxG.state, FreeplayState)) offset.set(25, 28);
		}
		return check;
	}
	...
	private function animationFinished(name:String)
	{
		switch(name)
		{
			case 'checking':
				...
				if (!Std.isOfType(FlxG.state, FreeplayState)) offset.set(3, 12);

			case 'unchecking':
				animation.play('unchecked', true);
				if (!Std.isOfType(FlxG.state, FreeplayState)) offset.set(0, 2);
		}
	}
}