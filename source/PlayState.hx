...
class PlayState extends MusicBeatState
{
	...
	public static var initialPlaylist:Int;
	...
	public static var campaignRating:Array<Float> = [];
	...
	override public function create()
	{
		...
		var showTime:Bool = (ClientPrefs.timeTxt);
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 58, 400, "", 32);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 2;
		timeTxt.visible = showTime;
		if(ClientPrefs.downScroll) timeTxt.y = 676;
		if(!ClientPrefs.downScroll && ClientPrefs.middleScroll) timeTxt.y = 10;
		
		showRating = ClientPrefs.ratingsEnabled;
		showComboNum = ClientPrefs.ratingsEnabled;
		...
		scoreTxt = new FlxText(0, 656.8, FlxG.width, "", 20);
		...
		botplayTxt = new FlxText(950, 17, FlxG.width - 800, "BOTPLAY", 32);
		...
		FlxTween.tween(LoadingState.blackScreen, {alpha: 1}, 2, {ease: FlxEase.linear});
	...
	function createPost()
	{
		switch(curSong)
		{
			case "Dragged Down To Hell": //this shit did NOT let me set it in lua and idfk why so im doing it here
				for (strum in opponentStrums) strum.texture = 'HellGates/HellNoteAssets3';
				for (note in unspawnNotes) if (!note.mustPress) note.texture = 'HellGates/HellNoteAssets3';
			
			case "Second Coming": //once again it did NOT let me do this in lua??

			//	for (strum in opponentStrums) strum.visible = false;

			//	playerStrums.members[0].x = 410;
			//	playerStrums.members[1].x = 522;
			//	playerStrums.members[2].x = 633;
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				campaignRating.push(ratingPercent); //please work oh my god PLEASE
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						...
						if (SONG.validScore)
						{
							...
							for (i in campaignRating)
							{
								weekRating += i;
							}
							weekRating = weekRating / initialPlaylist;
							Highscore.saveWeekRating(WeekData.getWeekFileName(), weekRating, storyDifficulty);
						}
	...
	override function beatHit()
	{
		...
		if (curSong == 'Devotion')
		{
			...
			{
				//case 576:
				//startVideo('SpookersD3End1'); nvm im doing this in lua cause this fucking kills the player for some reason wtfr???
