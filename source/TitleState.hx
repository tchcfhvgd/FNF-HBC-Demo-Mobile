...
var camFollow:FlxObject; // Using this for transition to main menu
...
//# PRECACHING MENU ^
		...
		FlxG.save.bind('funkin', '.bakugo/FallenAngels/ninjamuffin99');
	...
	function startIntro()
	{
		...
		logoBl.frames = Paths.getSparrowAtlas('logo_lol');

		logoBl.antialiasing = ClientPrefs.globalAntialiasing;
		logoBl.animation.addByPrefix('bump', 'bumpin logo', 24, false);
		...
		if (initialized) {
			camFollow.y -= 1000;
			camFollowPos.y -= 1000;
			FlxTween.tween(camFollow, {y: camFollowPos.y + 1000}, 1.3, {type: ONESHOT, ease: FlxEase.cubeInOut});
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxTween.tween(vignette, {alpha: 1}, 0.01, {onComplete:
					function(twn:FlxTween) {
						FlxTween.tween(vignette, {alpha: 0}, 1.3);
					}}
				);
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (mustUpdate) {
						...
						//camFollow.setPosition(camFollowPos.x, camFollowPos.y - 1000);
						//FlxTween.num(camFollow.y, camFollowPos.y - 500, 1, {ease: FlxEase.cubeInOut});
						FlxTween.tween(camFollow, {y: camFollowPos.y - 1000}, 1.6, {type: ONESHOT, ease: FlxEase.cubeInOut});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.visible = true;
				case 11:
					addMoreText('Hellbeats');
				// credTextShit.text += '\nNight';
				case 12:
					addMoreText('Corruption'); // credTextShit.text += '\nFunkin';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				remove(ngSpr);
				remove(credGroup);
				if (!MainMenuState.visitedMenu) FlxG.camera.flash(FlxColor.WHITE, 1.5);
